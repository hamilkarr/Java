...
public class BookManager {
	...
	static <K, V> K getKey(HashMap<K, V> map, String value) {
	...
	void searchBook() {
		System.out.println(getKey(bookList, "완전한 행복"));
		String key;
		System.out.print("검색어를 입력하시오: ");
		key = scan.nextLine();
		String searchKey = getKey(bookList, key);
		if (bookList.containsKey(searchKey)) {
			Book book = bookList.get(searchKey);
			System.out.println("검색 결과 -> " + book.toString());
		} else {
	...
	// private String getKey(HashMap<String, Book> bookList2, String key) {
	// return null;
	// }
